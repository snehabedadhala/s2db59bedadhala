...
exports.whiskey_update_Page =  async function(req, res) { 
    console.log("update view for item "+req.query.id) 
    try{ 
        let result = await whiskey.findById(req.query.id) 
        res.render('whiskeyupdate', { title: 'Whiskey Update', toShow: result }); 
    } 
    catch(err){ 
        res.status(500) 
        res.send(`{'error': '${err}'}`); 
...
exports.whiskey_delete_Page = async function(req, res) { 
    console.log("Delete view for id "  + req.query.id) 
    try{ 
        result = await whiskey.findById(req.query.id) 
        res.render('whiskeydelete', { title: 'whiskey Delete', toShow: result }); 
    } 
    catch(err){ 
        res.status(500) 
        res.send(`{'error': '${err}'}`); 
